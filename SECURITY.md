...
Report privately through [GitHub Security Advisories](https://github.com/stufently/telegram-ai-cli/security/advisories/new) on this repository. Please do not open a public issue for a vulnerability, and please do not report it by email — advisories are the only channel this project monitors for security reports.
...
- Text from a Telegram message, chat title or display name that changes the behaviour of this program, rather than being treated as inert data — including anything that alters what appears on a terminal reviewing a plan (see [`render.py`](telegram_ai_cli/render.py)).
...
- **A hard denylist, not a default.** `777000` and Saved Messages are excluded in [`config.py`](telegram_ai_cli/config.py) as constants, checked first in [`safety.py`](telegram_ai_cli/safety.py) before any allowlist is consulted. No YAML value and no environment variable can reopen them.
- **Fail-closed allow lists.** An empty `allow` list means nothing is permitted for that capability — reading a direct message, sending, joining, admin actions — except for reading groups and channels, which is open by default so the tool is usable before it has been configured at all. See the module docstrings in [`config.py`](telegram_ai_cli/config.py) and [`safety.py`](telegram_ai_cli/safety.py).
