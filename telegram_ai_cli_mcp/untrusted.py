...
in three separate places. :mod:`telegram_ai_cli_mcp.render` protects the *terminal*
(escape sequences that redraw the line a person is about to approve).
:mod:`telegram_ai_cli_mcp.redact` protects *privacy* (values recognisable by shape:
...
def wrap_untrusted(value: Any) -> Any:
    ...
    ``telegram_ai_cli_mcp.ops._common.telegram_result``), for the same reason
