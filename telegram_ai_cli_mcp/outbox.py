...
:mod:`telegram_ai_cli_mcp.ops.media` refuses to let a caller choose where a
...
def _require_private_root(root: Path) -> None:
    ...
    (:func:`telegram_ai_cli_mcp.ops.media._create_artifact`) and the archive
    (:func:`telegram_ai_cli_mcp.archive._narrow`) already treat their directories;
...
def resolve_outbound(
    ...
    *,
    ...
    # then compare on path components. See `telegram_ai_cli_mcp.roots`.
