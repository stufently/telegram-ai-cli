...
DEFAULT_CONFIG_PATH = Path(
    os.environ.get("TGAI_CONFIG", Path.home() / ".config" / "telegram-ai-cli-mcp" / "tgai.yaml")
)
...
def _state_home() -> Path:
    ...
    return root / "telegram-ai-cli-mcp"
...
class PathsConfig(BaseModel):
    ...
    #: reasoning is in `telegram_ai_cli_mcp/archive.py` and `docs/configuration.md`.
...
class McpConfig(BaseModel):
    ...
    **Set to an empty list, it publishes nothing** — the same reading every
    ...
    of the per-peer rules in :mod:`telegram_ai_cli_mcp.safety`, not a replacement
    for them.
    """
...
class TranscribeConfig(BaseModel):
    ...
    image: str = "telegram-ai-cli-mcp-transcribe:latest"
