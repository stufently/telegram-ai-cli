...
:attr:`~telegram_ai_cli_mcp.opspec.Effect.LOCAL_ADMIN`, which the registry's
...
:mod:`telegram_ai_cli_mcp.accounts.api_profile`. The two-step verification password
is never accepted from anywhere but an interactive prompt, which is
:mod:`telegram_ai_cli_mcp.accounts.login`'s own rule.
...
def _describe(view: AccountView) -> dict[str, Any]:
    ...
    :class:`~telegram_ai_cli_mcp.accounts.views.AccountView` is already free of
...
async def handle_account_login(ctx: OperationContext, params: LoginInput) -> Envelope:
    ...
    Idempotent by way of :func:`~telegram_ai_cli_mcp.accounts.login.interactive_login`:
